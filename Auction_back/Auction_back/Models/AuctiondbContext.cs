...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
        => optionsBuilder.UseSqlServer("Data Source=DESKTOP-UDRLDTQ; Initial Catalog=Auctiondb; Integrated Security=True; Pooling=False; Encrypt=False; TrustServerCertificate=True");

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Auction>(entity =>
        {
            entity.HasOne(d => d.MetaAuctionType).WithMany(p => p.Auctions)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_Auction_MetaAuctionType");
        });
        ...
        {
            entity.HasOne(d => d.CategoryHeader).WithMany(p => p.InverseCategoryHeader).HasConstraintName("FK_MetaCategory_MetaCategory");
        });
